...
type RoadEvent = {
  ...
  shortcut: { label: string; detail: string; toast: string; time: number };
};
...
const ROAD_EVENTS: Record<string, RoadEvent> = {
  ruin: {
    ...
    shortcut: { label: "Cross the collapsed wall", detail: "Get home 20s sooner", toast: "The broken wall saves time.", time: -20_000 },
  },
  forest: {
    ...
    shortcut: { label: "Wade through", detail: "Get home 25s sooner", toast: "The flooded middle is cold, but quick.", time: -25_000 },
  },
  road: {
    ...
    shortcut: { label: "Take the side stair", detail: "Get home 30s sooner", toast: "The hidden stair cuts across the keep.", time: -30_000 },
  },
  ash: {
    ...
    shortcut: { label: "Run the cooling ridge", detail: "Get home 20s sooner", toast: "The ridge holds long enough to cross.", time: -20_000 },
  },
};
...
const FALLBACK_EVENT: RoadEvent = {
  ...
  shortcut: { label: "Take the shortcut", detail: "Get home 25s sooner", toast: "Shortcut taken.", time: -25_000 },
};
  ...
  const choose = (choice: ChoiceId) => {
    ...
    if (choice === "investigate") {
      ...
      useKindling.setState({
        ...
        updatedAt,
        lastToast: `${event.investigate.toast} ${find.name} · +${Math.round(event.investigate.time / 1000)}s`,
      });
    } else if (choice === "rest") {
      const companion = current.companion
        ? { ...current.companion, bondXp: current.companion.bondXp + event.rest.bond }
        : null;
      useKindling.setState({
        companion,
        ...
        updatedAt,
        lastToast: `${event.rest.toast} · +${event.rest.bond} Bond XP · +${Math.round(event.rest.time / 1000)}s`,
      });
    } else {
      ...
      useKindling.setState({
        ...
        updatedAt,
        lastToast: `${event.shortcut.toast} · −${seconds}s`,
      });
    }
    ...
    <section className="fixed inset-x-3 bottom-20 z-40 mx-auto max-w-md rounded-xl border border-fire/35 bg-night/95 p-3 shadow-2xl backdrop-blur">
      ...
      <div className="mt-3 grid gap-2">
        <button type="button" onClick={() => choose("investigate")} className="flex min-h-11 items-center gap-3 rounded-md border border-bone/15 bg-stone px-3 text-left">
          ...
          <span className="flex-1"><span className="block text-sm font-medium">{event.investigate.label}</span><span className="block text-xs text-mute">{event.investigate.detail}</span></span>
        </button>
        <button type="button" onClick={() => choose("rest")} className="flex min-h-11 items-center gap-3 rounded-md border border-bone/15 bg-stone px-3 text-left">
          ...
          <span className="flex-1"><span className="block text-sm font-medium">{event.rest.label}</span><span className="block text-xs text-mute">{event.rest.detail}</span></span>
        </button>
        <button type="button" onClick={() => choose("shortcut")} className="flex min-h-11 items-center gap-3 rounded-md border border-bone/15 bg-stone px-3 text-left">
          ...
          <span className="flex-1"><span className="block text-sm font-medium">{event.shortcut.label}</span><span className="block text-xs text-mute">{event.shortcut.detail}</span></span>
        </button>
      </div>
    </section>
  );
